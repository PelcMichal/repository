...
public class Hello {
	...
	*/
	...
	public String saySoucetXML(@QueryParam("cislo1")int cislo1,@QueryParam("cislo2")int cislo2)
	{
		...
		try (ResultSet rs =GetConn().createStatement().executeQuery("select cislo1,cislo2,time from con order by time;");) {
				while(rs.next())
				{
					sb.append("<cislo1>");
					sb.append(rs.getInt(1));
					sb.append("</cislo1>");
					cal.setA(rs.getInt(1));
					sb.append("<cislo2>");
					sb.append(rs.getInt(2));
					sb.append("</cislo2>");
					cal.setB(rs.getInt(2));
					cal.add();
					sb.append("<soucet>");
					sb.append(cal.getC());
					sb.append("</soucet>");
					sb.append("<time>");
					sb.append(rs.getTimestamp(3));
					sb.append("</time>");
				}
		} catch (SQLException e) {
			LOG.error(e);
